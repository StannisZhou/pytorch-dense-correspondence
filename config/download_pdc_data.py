...
if len(sys.argv) < 2:
    ...
    print("for example:")
    print(
        "    python download_pdc_data.py /path/to/pytorch-dense-correspondence/config/dense_correspondence/dataset/composite/caterpillar_upright.yaml"
    )
    quit()


logs = []
datasets_to_download_config = yaml.load(open(sys.argv[1]), Loader=yaml.FullLoader)
...
def add_datset(path):
    global logs
    single_object_dataset = yaml.load(open(path), Loader=yaml.FullLoader)
...
for i in datasets_to_download_config['single_object_scenes_config_files']:
    path = os.path.join(
        pdc_root_dir, "config/dense_correspondence/dataset/single_object", i
    )
    add_datset(path)

for i in datasets_to_download_config['multi_object_scenes_config_files']:
    path = os.path.join(
        pdc_root_dir, "config/dense_correspondence/dataset/multi_object", i
    )
    add_datset(path)
...
if not os.path.isdir(os.path.join(pdc_data_dir, "evaluation_labeled_data")):
    ...
    os.system(
        "wget -q " + download_url_base + "evaluation_labeled_data_compressed.tar.gz"
    )
    evaluation_data_compressed_path = os.path.join(
        data_dir, "evaluation_labeled_data_compressed.tar.gz"
    )
