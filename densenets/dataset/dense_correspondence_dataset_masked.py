...
class DenseCorrespondenceDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.debug:
            ...
            indexes_to_keep = (
                (torch.rand(num_matches_to_plot) * len(uv_a[0]))
                .floor()
        ...
        if index % 2:
            ...
            image_b_mask = torch.from_numpy(np.asarray(image_b_mask)).type(
                torch.FloatTensor
            )
        else:
        ...
        if self.debug:
            ...
            uv_a_long = (
                torch.t(uv_a[0].repeat(self.num_non_matches_per_match, 1))
                .contiguous()
                .view(-1, 1),
                torch.t(uv_a[1].repeat(self.num_non_matches_per_match, 1))
                .contiguous()
                .view(-1, 1),
            )
        ...
        non_matches_b = (
            uv_b_non_matches_long[1].long() * image_width
            + uv_b_non_matches_long[0].long()
        )
    ...
    def get_img_idx_with_different_pose(
        self, scene_name, pose_a, threshold=0.2, angle_threshold=20, num_attempts=10
    ):
        """
    ...
    def use_all_available_scenes(self):
        self.scenes = [
            os.path.basename(x) for x in glob.glob(self.logs_root_path + "*")
        ]
    ...
    def set_parameters_from_training_config(self, training_config):
        ...
        self.sample_matches_only_off_mask = training_config['training'][
            'sample_matches_only_off_mask'
        ]

        self.num_non_matches_per_match = training_config['training'][
            "num_non_matches_per_match"
        ]

        self.num_masked_non_matches_per_match = int(
            training_config['training']["fraction_masked_non_matches"]
            * self.num_non_matches_per_match
        )

        self.num_background_non_matches_per_match = int(
            training_config['training']["fraction_background_non_matches"]
            * self.num_non_matches_per_match
        )

        self.cross_scene_num_samples = training_config['training'][
            "cross_scene_num_samples"
        ]

        self._use_image_b_mask_inv = training_config["training"]["use_image_b_mask_inv"]

        from densenets.dataset.spartan_dataset_masked import SpartanDatasetDataType

        self._data_load_types = []
        self._data_load_type_probabilities = []

        p = training_config["training"]["data_type_probabilities"][
            "SINGLE_OBJECT_WITHIN_SCENE"
        ]
        if p > 0:
        ...
        self._data_load_type_probabilities = np.divide(
            self._data_load_type_probabilities,
            np.sum(self._data_load_type_probabilities),
        )
