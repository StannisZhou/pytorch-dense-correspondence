...
import densenets.dense_correspondence_manipulation.utils.segmentation as segmentation
...
from densenets.dense_correspondence_manipulation.utils.constants import *
from director import filterUtils, imageview, ioUtils, transformUtils, viewbehaviors
...
class ElasticFusionCameraPoses(FusionCameraPoses):
    ...
    def get_camera_to_world_pose(self, idx):
        ...
        camera_to_world = transformUtils.concatenateTransforms(
            [camera_to_first_frame, self.first_frame_to_world]
        )
...
class FusionReconstruction(object):
    ...
    def visualize_reconstruction(self, view, point_size=None, vis_uncropped=False):
        ...
        self.reconstruction_vis_obj = vis.updatePolyData(
            self.poly_data, 'Fusion Reconstruction', view=view, colorByName='RGB'
        )
        self.reconstruction_vis_obj.setProperty('Point Size', point_size)

        if vis_uncropped:
            vis_obj = vis.updatePolyData(
                self.poly_data_uncropped,
                'Uncropped Fusion Reconstruction',
                view=view,
                colorByName='RGB',
            )
...
class TSDFReconstruction(FusionReconstruction):
    ...
    def load_poly_data(self):
        ...
        if self._load_foreground_mesh:
            ...
            if not os.path.isfile(foreground_reconstruction_filename):
                print "Foreground mesh file doesn't exist, falling back" " to cropping mesh"
                self.crop_poly_data()
            else:
    ...
    def visualize_reconstruction(
        self, view, vis_uncropped=False, name=None, parent=None
    ):
        ...
        self.reconstruction_vis_obj = vis.updatePolyData(
            self.poly_data, vis_name, view=view, colorByName='RGB', parent=parent
        )
    ...
    def from_data_folder(
        data_folder, config=None, name=None, load_foreground_mesh=True
    ):
        """
