import numpy as np
import PythonQt
from director import imageview, viewbehaviors
...
def vtk_image_to_numpy_array(vtk_image):
    ...
    scalars = vnp.getNumpyFromVtk(
        vtk_image, vtk_image.GetPointData().GetScalars().GetName()
    )
...
def computeDepthImageAndPointCloud(depthBuffer, colorBuffer, camera):
    ...
    vtk.vtkDepthImageUtils.DepthBufferToDepthImage(
        depthBuffer, colorBuffer, camera, depthImage, pts, ptColors
    )
...
class DepthScanner(object):
    ...
    def update(self):
        ...
        if not self.pointCloudObj:
            self.pointCloudObj = vis.showPolyData(
                polyData, 'point cloud', colorByName='rgb', view=self.pointCloudView
            )
        else:
    ...
    def getDepthImageAndPointCloud(self):
        return computeDepthImageAndPointCloud(
            self.getDepthBufferImage(), self.getColorBufferImage(), self.view.camera()
        )
...
def main(globalsDict=None):
    ...
    dock = app.app.addWidgetToDock(
        depthScanner.imageView.view, QtCore.Qt.RightDockWidgetArea
    )
    dock.setMinimumWidth(300)
    dock.setMinimumHeight(300)

    dock = app.app.addWidgetToDock(
        depthScanner.pointCloudView, QtCore.Qt.RightDockWidgetArea
    )
    dock.setMinimumWidth(300)
    dock.setMinimumHeight(300)
