...
import densenets.dense_correspondence_manipulation.utils.transformations as transformations
import numpy as np
import torch
...
def getDenseCorrespondenceSourceDir():
    ...
    return '/home/stannis/ltr/sandbox/stannis/densenets/'


def get_data_dir():
    #  return os.getenv("DC_DATA_DIR")
    return '/home/stannis/Downloads/dense_correspondence_data/pdc'
...
def add_dense_correspondence_to_python_path():
    ...
    sys.path.append(
        os.path.join(dc_source_dir, 'external/pytorch-segmentation-detection')
    )
...
def pil_image_to_cv2(pil_image):
    ...
    return np.array(pil_image)[
        :, :, ::-1
    ].copy()  # open and convert between BGR and RGB
