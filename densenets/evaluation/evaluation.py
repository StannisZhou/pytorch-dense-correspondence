...
import cv2
import densenets.correspondence_tools.correspondence_finder as correspondence_finder
import densenets.correspondence_tools.correspondence_plotter as correspondence_plotter
...
import densenets.evaluation.plotting as dc_plotting
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import scipy.stats as ss
import torch
from densenets import transforms
from densenets.correspondence_tools.correspondence_finder import (
    random_sample_from_masked_image,
)
from densenets.dataset.spartan_dataset_masked import SpartanDataset
from densenets.dense_correspondence_manipulation.utils import constants
from densenets.evaluation.utils import PandaDataFrameWrapper
from densenets.loss_functions.pixelwise_contrastive_loss import PixelwiseContrastiveLoss
...
class SIFTKeypointMatchPandaTemplate(PandaDataFrameWrapper):
    columns = ['scene_name', 'img_a_idx', 'img_b_idx', 'is_valid', 'norm_diff_pred_3d']
...
class DenseCorrespondenceEvaluation(object):
    ...
    def get_output_dir(self):
        return utils.convert_data_relative_path_to_absolute_path(
            self._config['output_dir']
        )

    @staticmethod
    def get_image_pair_with_poses_diff_above_threshold(
        dataset, scene_name, threshold=0.05, max_num_attempts=100
    ):
        """
    ...
    def evaluate_single_network(self, network_name, mode="train", save=True):
        ...
        num_matches_per_image_pair = self._config['params'][
            'num_matches_per_image_pair'
        ]
    ...
    def evaluate_single_network_cross_scene(self, network_name, save=True):
        ...
        DenseCorrespondenceEvaluation.evaluate_network_cross_scene(
    ...
    def evaluate_network_cross_scene(dcn=None, dataset=None, save=True):
        ...
        for annotated_pair in cross_scene_data:
            ...
            if not os.path.isdir(
                dataset.get_full_path_for_scene(scene_name_a)
            ) or not os.path.isdir(dataset.get_full_path_for_scene(scene_name_b)):
                print(
                    "at least one of these scene names does not exist:",
                    scene_name_a,
                    scene_name_b,
                )
                continue
            ...
            dataframe_list_temp = DenseCorrespondenceEvaluation.single_cross_scene_image_pair_quantitative_analysis(
                dcn,
                dataset,
                scene_name_a,
                image_a_idx,
    ...
    def evaluate_network_across_objects(dcn=None, dataset=None, num_image_pairs=25):
        ...
        for i in range(num_image_pairs):
            ...
            dataframe_list_temp = DenseCorrespondenceEvaluation.single_across_object_image_pair_quantitative_analysis(
                dcn,
                dataset,
                scene_name_a,
                scene_name_b,
    ...
    def evaluate_single_network_cross_instance(
        self, network_name, full_path_cross_instance_labels, save=False
    ):
        """
        Simple wrapper that uses class config and then calls static method
        """
        ...
        dcn.eval()
        dataset = dcn.load_training_dataset()
        return DenseCorrespondenceEvaluation.evaluate_network_cross_instance(
            dcn, dataset, full_path_cross_instance_labels, save=False
        )

    @staticmethod
    def evaluate_network_cross_instance(
        dcn=None, dataset=None, full_path_cross_instance_labels=None, save=False
    ):
        """
    ...
    @staticmethod
    def evaluate_network_cross_scene_keypoints(
        dcn, dataset, full_path_cross_instance_labels
    ):
        """
        ...
        for subset in itertools.combinations(cross_instance_keypoint_labels, 2):
            ...
            dataframe_list_temp = DenseCorrespondenceEvaluation.single_image_pair_cross_scene_keypoints_quantitative_analysis(
    ...
    def evaluate_network(
        ...
        for i in range(0, num_image_pairs):
            ...
            if i % logging_rate == 0:
                print(
                    f"computing statistics for image {i} of {num_image_pairs}, scene_name {scene_name}"
                )
                print("scene")

            idx_pair = DCE.get_image_pair_with_poses_diff_above_threshold(
                dataset, scene_name
            )
    ...
    def clip_pixel_to_image_size_and_round(uv, image_width, image_height):
        ...
        u = min(
            int(round(uv[0]) if isinstance(uv[0], float) else torch.round(uv[0])),
            image_width - 1,
        )
        v = min(
            int(round(uv[1]) if isinstance(uv[1], float) else torch.round(uv[1])),
            image_height - 1,
        )
    ...
    @staticmethod
    def single_cross_scene_image_pair_quantitative_analysis(
        dcn,
        dataset,
        scene_name_a,
        img_a_idx,
        ...
        for i in range(len(img_a_pixels)):
            ...
            pd_template = (
                DenseCorrespondenceEvaluation.compute_descriptor_match_statistics(
                    depth_a,
                    depth_b,
                    mask_a,
                    mask_b,
        ...
        for i in range(len(img_a_pixels)):
            ...
            for j in range(J):
                ...
                (
                    ...
                    diff_mask_a,
                    diff_pose_a,
                ) = dataset.get_rgbd_mask_pose(scene_name_a, different_view_a_idx)
                ...
                (
                    ...
                    depth_a, pose_a, diff_depth_a, diff_pose_a, uv_a=uv_a
                )
                if uv_a_vec is None:
                ...
                diff_res_a = (
                    dcn.forward_single_image_tensor(diff_rgb_a_tensor)
                    .data.cpu()
                    .numpy()
                )

                diff_uv_a = (diff_uv_a_vec[0][0], diff_uv_a_vec[1][0])
                diff_uv_a = DCE.clip_pixel_to_image_size_and_round(
            ...
            for k in range(K):
                ...
                (
                    ...
                    diff_mask_b,
                    diff_pose_b,
                ) = dataset.get_rgbd_mask_pose(scene_name_b, different_view_b_idx)
                ...
                (
                    ...
                    depth_b, pose_b, diff_depth_b, diff_pose_b, uv_a=uv_b
                )
                if uv_b_vec is None:
                    logging.info("no matches found, continuing")
                    continue
                ...
                diff_res_b = (
                    dcn.forward_single_image_tensor(diff_rgb_b_tensor)
                    .data.cpu()
                    .numpy()
                )

                diff_uv_b = (diff_uv_b_vec[0][0], diff_uv_b_vec[1][0])
                diff_uv_b = DCE.clip_pixel_to_image_size_and_round(
    ...
    @staticmethod
    def single_across_object_image_pair_quantitative_analysis(
        dcn,
        dataset,
        scene_name_a,
        scene_name_b,
        ...
        for i in range(num_uv_a_samples):
            ...
            pd_template = DCE.compute_descriptor_match_statistics_no_ground_truth(
                uv_a,
                res_a,
                res_b,
                rgb_a=rgb_a,
    ...
    def single_same_scene_image_pair_quantitative_analysis(
        ...
        (uv_a_vec, uv_b_vec) = correspondence_finder.batch_find_pixel_correspondences(
            depth_a, pose_a, depth_b, pose_b, device='CPU', img_a_mask=mask_a
        )
    ...
    @staticmethod
    def compute_descriptor_match_statistics_no_ground_truth(
        uv_a,
        res_a,
        res_b,
        rgb_a=None,
        ...
        # compute best match
        uv_b, best_match_diff, norm_diffs = DenseCorrespondenceNetwork.find_best_match(
            uv_a, res_a, res_b, debug=debug
        )

        if debug:
            correspondence_plotter.plot_correspondences_direct(
                rgb_a, depth_a, rgb_b, depth_b, uv_a, uv_b, show=True
            )
    ...
    def compute_descriptor_match_statistics(
        ...
        uv_a_depth = (
            depth_a[int(uv_a[1]), int(uv_a[0])] / constants.DEPTH_IM_SCALE
        )  # check if this is not None
        uv_b_depth = depth_b[int(uv_b[1]), int(uv_b[0])] / constants.DEPTH_IM_SCALE
        uv_b_pred_depth = depth_b[uv_b_pred[1], uv_b_pred[0]] / constants.DEPTH_IM_SCALE
        uv_b_pred_depth_is_valid = DenseCorrespondenceEvaluation.is_depth_valid(
            uv_b_pred_depth
        )
        uv_b_pred_depth_masked = (
            depth_b[uv_b_pred_masked[1], uv_b_pred_masked[0]] / constants.DEPTH_IM_SCALE
        )
        uv_b_pred_depth_is_valid_masked = DenseCorrespondenceEvaluation.is_depth_valid(
            uv_b_pred_depth_masked
        )
        ...
        uv_b_pred_pos = DCE.compute_3d_position(
            uv_b_pred, uv_b_pred_depth, camera_matrix, pose_b
        )
        uv_b_pred_pos_masked = DCE.compute_3d_position(
        ...
        if DCE.is_depth_valid(uv_b_depth) and DCE.is_depth_valid(
            uv_b_pred_depth_masked
        ):
        ...
        if debug:
            ...
            fig, axes = correspondence_plotter.plot_correspondences_direct(
                rgb_a, depth_a, rgb_b, depth_b, uv_a, uv_b, show=False
            )
    ...
    def compute_3d_position(uv, depth, camera_intrinsics_matrix, camera_to_world):
        ...
        pos_in_world_frame = np.dot(camera_to_world, np.append(pos_in_camera_frame, 1))[
            :3
        ]
    ...
    @staticmethod
    def single_same_scene_image_pair_qualitative_analysis(
        dcn, dataset, scene_name, img_a_idx, img_b_idx, num_matches=10
    ):
        """
        ...
        DenseCorrespondenceEvaluation.single_image_pair_qualitative_analysis(
            dcn, dataset, rgb_a, rgb_b, mask_a, mask_b, num_matches
        )

    @staticmethod
    def single_cross_scene_image_pair_qualitative_analysis(
        dcn, dataset, scene_name_a, img_a_idx, scene_name_b, img_b_idx, num_matches=10
    ):
        """
    ...
    @staticmethod
    def single_image_pair_keypoint_qualitative_analysis(
        dcn,
        dataset,
        keypoint_data_a,
        keypoint_data_b,
    ...
    @staticmethod
    def single_image_pair_cross_scene_keypoints_quantitative_analysis(
        dcn, dataset, keypoint_data_a, keypoint_data_b, res_a=None, res_b=None
    ):
        """
        ...
        for kp_name, data_a in keypoint_data_a['keypoints'].items():
            if kp_name not in keypoint_data_b['keypoints']:
                raise ValueError(
                    "keypoint %s appears in one list of annotated data but"
            ...
            for order in ordering:
                ...
                uv_1 = DCE.clip_pixel_to_image_size_and_round(
                    (data[idx_1]['u'], data[idx_2]['v']), image_width, image_height
                )
                uv_2 = DCE.clip_pixel_to_image_size_and_round(
                    (data[idx_2]['u'], data[idx_2]['v']), image_width, image_height
                )
    ...
    @staticmethod
    def single_image_pair_sift_analysis(
        dataset,
        scene_name,
        img_a_idx,
        img_b_idx,
        ...
        for idx, val in enumerate(good):
            ...
            df = DCE.compute_single_sift_match_statistics(
                depth_a, depth_b, kp_a, kp_b, pose_a, pose_b, camera_intrinsics_matrix
            )
    ...
    @staticmethod
    def compute_single_sift_match_statistics(
        depth_a,
        depth_b,
        kp_a,
        kp_b,
        ...
        uv_a_depth = depth_a[uv_a[1], uv_a[0]] / constants.DEPTH_IM_SCALE
        # print "uv_a", uv_a
        ...
        uv_b_depth = depth_b[uv_b[1], uv_b[0]] / constants.DEPTH_IM_SCALE
    ...
    @staticmethod
    def single_image_pair_keypoint_analysis(
        dcn,
        dataset,
        scene_name,
        img_a_idx,
        ...
        sift_data = DCE.single_image_pair_sift_analysis(
            dataset, scene_name, img_a_idx, img_b_idx, visualize=visualize
        )
    ...
    def parse_cross_scene_data(dataset):
        ...
        # case?
        evaluation_labeled_data_paths += dataset.config["multi_object"][
    ...
    @staticmethod
    def evaluate_network_qualitative_cross_scene(
        dcn, dataset, draw_human_annotations=True
    ):
        """
        This will search for the "evaluation_labeled_data_path" in the dataset.yaml,
        and use pairs of images that have been human-labeled across scenes.
    ...
    def evaluate_network_qualitative(
        ...
        if randomize:
            (
                scene_names,
                img_pairs,
            ) = DenseCorrespondenceEvaluation.get_random_scenes_and_image_pairs(dataset)
        else:
            if scene_type == "caterpillar":
                ...
                raise ValueError(
                    "scene_type must be one of [drill, caterpillar], it was %s)"
                    % (scene_type)
                )
        ...
        for scene_name, img_pair in zip(scene_names, img_pairs):
            ...
            DenseCorrespondenceEvaluation.single_same_scene_image_pair_qualitative_analysis(
        ...
        if scene_type == "caterpillar":
            ...
            if randomize:
                (
                    scene_name,
                    img_pairs,
                ) = DenseCorrespondenceEvaluation.get_random_image_pairs(dataset)
            else:
    ...
    @staticmethod
    def run_cross_instance_keypoint_evaluation_on_network(
        model_folder,
        path_to_cross_instance_labels,
        ...
        df = DCE.evaluate_network_cross_scene_keypoints(
    ...
    def make_2d_cluster_plot(dcn, dataset, plot_background=False):
        ...
        for i in range(100):
            ...
            for j in range(len(object_u_samples)):
                descriptors_object[j, :] = res[
                    object_v_samples[j], object_u_samples[j], :
                ]
            if use_3d:
                ...
                for j in range(len(object_u_samples)):
                    descriptors_object_xy[j, :] = res[
                        object_v_samples[j], object_u_samples[j], 0:2
                    ]
                    descriptors_object_yz[j, :] = res[
                        object_v_samples[j], object_u_samples[j], 1:3
                    ]
                    descriptors_object_xz[j, :] = res[
                        object_v_samples[j], object_u_samples[j], 0::2
                    ]

            descriptors_background = np.zeros((len(background_u_samples), d))
            for j in range(len(background_u_samples)):
                descriptors_background[j, :] = res[
                    background_v_samples[j], background_u_samples[j], :
                ]
            if use_3d:
                ...
                for j in range(len(background_u_samples)):
                    descriptors_background_xy[j, :] = res[
                        background_v_samples[j], background_u_samples[j], 0:2
                    ]
                    descriptors_background_yz[j, :] = res[
                        background_v_samples[j], background_u_samples[j], 1:3
                    ]
                    descriptors_background_xz[j, :] = res[
                        background_v_samples[j], background_u_samples[j], 0::2
                    ]
            ...
            if object_id not in descriptors_known_objects_samples:
                ...
                if use_3d:
                    descriptors_known_objects_samples_xy[
                        object_id
                    ] = descriptors_object_xy
                    descriptors_known_objects_samples_yz[
                        object_id
                    ] = descriptors_object_yz
                    descriptors_known_objects_samples_xz[
                        object_id
                    ] = descriptors_object_xz
            else:
                ...
                if use_3d:
                    descriptors_known_objects_samples_xy[object_id] = np.vstack(
                        (
                            descriptors_known_objects_samples_xy[object_id],
                            descriptors_object_xy,
                        )
                    )
                    descriptors_known_objects_samples_yz[object_id] = np.vstack(
                        (
                            descriptors_known_objects_samples_yz[object_id],
                            descriptors_object_yz,
                        )
                    )
                    descriptors_known_objects_samples_xz[object_id] = np.vstack(
                        (
                            descriptors_known_objects_samples_xz[object_id],
                            descriptors_object_xz,
                        )
                    )
        ...
        if not use_3d:
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples[:, 0],
                    descriptors_background_samples[:, 1],
                    alpha=0.5,
                    label="background",
                )
            plt.legend()
        ...
        if use_3d:
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples_xy[:, 0],
                    descriptors_background_samples_xy[:, 1],
                    alpha=0.5,
                    label="background",
                )
            plt.legend()
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples_yz[:, 0],
                    descriptors_background_samples_yz[:, 1],
                    alpha=0.5,
                    label="background",
                )
            plt.legend()
            ...
            if plot_background:
                plt.scatter(
                    descriptors_background_samples_xz[:, 0],
                    descriptors_background_samples_xz[:, 1],
                    alpha=0.5,
                    label="background",
                )
            plt.legend()
    ...
    def test(dcn, dataset, data_idx=1, visualize=False, debug=False, match_idx=10):
        ...
        DenseCorrespondenceEvaluation.single_same_scene_image_pair_qualitative_analysis(
            dcn, dataset, scene_name, img_idx_a, img_idx_b
        )
...
class DenseCorrespondenceEvaluationPlotter(object):
    ...
    def run_on_single_dataframe(
        ...
        if dataframe is None:
            path_to_csv = utils.convert_data_relative_path_to_absolute_path(
                path_to_df_csv, assert_path_exists=True
            )
        ...
        if use_masked_plots:
            plot = DCEP.make_descriptor_accuracy_plot(
        ...
        if use_masked_plots:
            plot = DCEP.make_fraction_false_positives_plot(
        ...
        if use_masked_plots:
            plot = DCEP.make_average_l2_false_positives_plot(
