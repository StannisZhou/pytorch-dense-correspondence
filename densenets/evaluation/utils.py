...
class KeypointAnnotationsPandasTemplate(PandaDataFrameWrapper):
    columns = ['scene_name', 'image_idx', 'keypoint_name', 'object_id', 'u', 'v']
...
def extract_descriptor_images_for_scene(
    dcn, dataset, scene_name, save_dir, overwrite=False
):
    """
    ...
    if os.path.exists(save_dir):
        if not overwrite:
            raise ValueError(
                "save_dir %s already exists and overwrite is False" % (save_dir)
            )
        else:
    ...
    for counter, img_idx in enumerate(image_idxs):
        ...
        descriptor_image_filename = (
            utils.getPaddedString(img_idx, width=SpartanDataset.PADDED_STRING_WIDTH)
            + "_descriptor.npy"
        )
