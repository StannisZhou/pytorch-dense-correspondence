...
import densenets.loss_functions.loss_composer as loss_composer
import tensorboard_logger
import torch
import torch.optim as optim
from densenets.dataset.spartan_dataset_masked import (
    SpartanDataset,
...
class DenseCorrespondenceTraining(object):
    ...
    def build_network(self):
        ...
        return DenseCorrespondenceNetwork.from_config(
            self._config['dense_correspondence_network'], load_stored_params=False
        )
    ...
    def run_from_pretrained(self, model_folder, iteration=None, learning_rate=None):
        ...
        if learning_rate is not None:
            self._config["training"][
                "learning_rate_starting_from_pretrained"
            ] = learning_rate
    ...
    def run(self, loss_current_iteration=0, use_pretrained=False):
        ...
        max_num_iterations = (
            self._config['training']['num_iterations'] + start_iteration
        )
        ...
        for epoch in range(50):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(self._data_loader, 0):
                ...
                masked_non_matches_a = Variable(
                    masked_non_matches_a.cuda().squeeze(0), requires_grad=False
                )
                masked_non_matches_b = Variable(
                    masked_non_matches_b.cuda().squeeze(0), requires_grad=False
                )

                background_non_matches_a = Variable(
                    background_non_matches_a.cuda().squeeze(0), requires_grad=False
                )
                background_non_matches_b = Variable(
                    background_non_matches_b.cuda().squeeze(0), requires_grad=False
                )
                ...
                def update_plots(
                    ...
                    if not loss_composer.is_zero_loss(masked_non_match_loss):
                        self._logging_dict['train']['masked_non_match_loss'].append(
                    ...
                    if not loss_composer.is_zero_loss(background_non_match_loss):
                        self._logging_dict['train']['background_non_match_loss'].append(
                    ...
                    if data_type == SpartanDatasetDataType.SINGLE_OBJECT_WITHIN_SCENE:
                        ...
                        self._tensorboard_logger.log_value(
                            "train loss SYNTHETIC_MULTI_OBJECT",
                            loss.item(),
                            loss_current_iteration,
                        )
                    else:
                ...
                if loss_current_iteration % logging_rate == 0:
                    ...
                    logging.info(
                        "Training is %d percent complete\n" % (percent_complete)
                    )

                # don't compute the test loss on the first few times through the loop
                if (
                    self._config["training"]["compute_test_loss"]
                    and (loss_current_iteration % compute_test_loss_rate == 0)
                    and loss_current_iteration > 5
                ):
                ...
                if (
                    loss_current_iteration
                    % self._config['training']['garbage_collect_rate']
                    == 0
                ):
                ...
                if loss_current_iteration > max_num_iterations:
                    logging.info(
                        "Finished testing after %d iterations" % (max_num_iterations)
                    )
                    self.save_network(
                        dcn,
                        optimizer,
    ...
    def setup_logging_dir(self):
        ...
        if 'logging_dir_name' in self._config['training']:
            ...
            dir_name = (
                utils.get_current_time_unique_name()
                + "_"
                + str(
                    self._config['dense_correspondence_network']['descriptor_dimension']
                )
                + "d"
            )
        ...
        self._logging_dir = os.path.join(
            utils.convert_data_relative_path_to_absolute_path(
                self._config['training']['logging_dir']
            ),
            dir_name,
        )
    ...
    def adjust_learning_rate(self, optimizer, iteration):
        ...
        if iteration % steps_between_learning_rate_decay == 0:
            for param_group in optimizer.param_groups:
                param_group['lr'] = (
                    param_group['lr'] * self._config["training"]["learning_rate_decay"]
                )
    ...
    def load_default_config():
        ...
        config_file = os.path.join(
            dc_source_dir, 'config', 'dense_correspondence', 'training', 'training.yaml'
        )
